...
### Git ブランチ運用ルール（スカッシュマージ前提）

#### マージ方式
- **develop へのマージ**: Squash and Merge（履歴をきれいに保つ）
- **main へのマージ**: Merge Commit（リリース履歴を追跡可能にする）

#### マージ後のブランチ運用（重要）
- **PRがマージされたブランチで作業を続けてはならない**
- スカッシュマージは元のコミット履歴を圧縮するため、マージ済みブランチで追加コミットすると develop との間でコンフリクトが発生する
- 追加修正が必要な場合は以下の手順に従う:
  1. `develop` を最新に pull する
  2. `develop` から新しいブランチを作成する（例: `fix/mindmap-ci`）
  3. 新しいブランチで修正を行い、新たに PR を作成する
