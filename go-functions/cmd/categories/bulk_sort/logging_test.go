...
func TestHandler_InternalFailureLogsCauseAndCorrelation(t *testing.T) {
	t.Setenv("CATEGORIES_TABLE_NAME", "categories")
	original := dynamoClientGetter
	t.Cleanup(func() { dynamoClientGetter = original })
	const cause = "private dependency endpoint unavailable"
	dynamoClientGetter = func() (DynamoDBClientInterface, error) { return nil, errors.New(cause) }
	request := events.APIGatewayProxyRequest{
		Body:           `{"orders":[{"id":"test","sortOrder":0}]}`,
