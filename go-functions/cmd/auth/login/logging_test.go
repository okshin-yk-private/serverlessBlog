...
func TestHandler_InternalFailureLogsCauseAndCorrelation(t *testing.T) {
	t.Setenv("USER_POOL_CLIENT_ID", "client")
	original := cognitoClientGetter
	t.Cleanup(func() { cognitoClientGetter = original })
	const cause = "private dependency endpoint unavailable"
	cognitoClientGetter = func() (CognitoClientInterface, error) { return nil, errors.New(cause) }
	request := events.APIGatewayProxyRequest{
		Body:           `{"email":"private@example.com","password":"private-password"}`,
