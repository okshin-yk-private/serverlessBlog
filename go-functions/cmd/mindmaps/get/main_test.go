...
func TestHandler_SuccessfulGet(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()
	...
	if result.Title != "Test Mindmap" {
		t.Errorf("Expected Title 'Test Mindmap', got %s", result.Title)
	}
}

func TestHandler_ForbiddenDraftByOtherUser(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()
...
func TestHandler_PublishedAccessibleByOtherUser(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()
...
func TestHandler_GetReturnsAllFields(t *testing.T) {
	cleanup := setupTest(t)
	defer cleanup()
	...
	resp, err := Handler(context.Background(), makeAuthRequest("mindmap-full"))
	if err != nil {
		t.Fatalf("Handler returned error: %v", err)
	}
	if resp.StatusCode != 200 {
	...
	if result.PublishedAt == nil || *result.PublishedAt != publishedAt {
		t.Errorf("Expected PublishedAt %s, got %v", publishedAt, result.PublishedAt)
	}
