...
| Infrastructure (Terraform) | ![Infrastructure Coverage](./docs/badges/coverage-infrastructure.svg) |
...
- **Terraform**: HCLによるインフラ定義 (~> 1.14)
- **Checkov**: IaCセキュリティスキャン

### ランタイム
- **Go 1.25.x**: Lambda関数実装（provided.al2023、ARM64）
...
│   │   └── prd/              # 本番環境
│   └── bootstrap/            # 状態バックエンド
├── go-functions/             # Go Lambda関数
│   ├── cmd/                  # エントリーポイント
│   │   ├── auth/             # 認証（login, logout, refresh）
│   │   ├── posts/            # 記事（CRUD）
│   │   ├── images/           # 画像（upload, delete）
│   │   └── categories/       # カテゴリ（CRUD, seed）
│   └── internal/             # 共通ライブラリ
├── frontend/
│   ├── public/               # 公開サイト（React 18 + Vite）
│   └── admin/                # 管理画面（React 18 + Vite）
...
```bash
# リポジトリのクローン
git clone https://github.com/okshin-yk-private/serverlessBlog.git
cd serverlessBlog
...
※ `.env` やTerraform state、AWS資格情報は削除されません。
...
詳細は [Terraformドキュメント](./terraform/README.md) を参照してください。
...
このプロジェクトは、AWS Serverless ArchitectureとTerraform、オープンソースコミュニティの貢献に基づいています。
