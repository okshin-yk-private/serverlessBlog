...
2. GetUserPoolMfaConfig の既存 MFA 設定を保持し、RP ID・本人確認必須・Factor を
   SetUserPoolMfaConfig で設定する。SMS を含む既存設定を送信し、読み戻しで全設定の
   一致を要求する。一致しなければ停止する。
...
2026-09-23 の [DEV deploy](https://github.com/okshin-yk-private/serverlessBlog/actions/runs/35828952546/job/107076974079)
は `SetUserPoolMfaConfig` の `InvalidSmsRoleTrustRelationshipException` で停止した。
CloudTrail と IAM の読み取りで、プールの SMS 設定が参照するロールが `NoSuchEntity`
であることを確認した。後続の Terraform `UpdateUserPool` も同じ参照を再送するため、
ヘルパーで SMS を省略するだけでは解決しない。

`terraform/environments/dev/cognito_sms_recovery.tf` は同名・同 ARN のロールを復元する。
信頼先を Cognito サービス、既存 ExternalId、現在のアカウント、SSM にある DEV プールの
ARN に制限する。送信権限は対象リージョンの `sns:Publish`。直接 SMS 送信はトピック
ARN を持たないため `Resource = "*"` が必要になる。これは既存 SMS 送信能力を復元する
変更であり、送信が利用されれば SMS 料金が発生し得る。TOTP 設定や利用者の MFA 選択は変更しない。
PRD のロール復元は含めない。

復元後も通常の全体 plan と MFA 読み戻しを必ず実行する。DEV のロール作成直後の IAM
伝播遅延に限り、SMS ロールの信頼・アクセス権エラーを最大6回、合計31秒の待機で再試行する。
解消しない場合は失敗とし、ログの操作名・AWS エラーコードで判断する。他のエラーは再試行しない。
リクエスト JSON、認証情報、AWS の生のエラー本文はログに出さない。

根拠: [AWS Cognito の SMS 用 IAM 設定](https://docs.aws.amazon.com/cognito/latest/developerguide/user-pool-sms-settings.html)、
[AWS Provider 6.66 の UpdateUserPool 実装](https://github.com/hashicorp/terraform-provider-aws/blob/v6.66.0/internal/service/cognitoidp/user_pool.go)。
