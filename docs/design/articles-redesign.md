...
この図は投稿行為の監査履歴ではない。削除・非公開化した記事は次のビルドで集計から外れる。集計は静的生成時点のスナップショット。基準日の表示は後続の簡略化で取り除いた。実データの `publishedAt` 完全性は今回照合していない。
...
デプロイ後のフィードバックに合わせ、Articlesの副題とカレンダーの集計日時・日別詳細・集計条件・集計対象外の説明を画面から削除した。見出しは `History`、色の凡例は `Less` から `More`、曜日は `Mon`〜`Sun` に変更した。未集計の凡例は表示しない。日付セルのツールチップ・読み上げ用ラベルと、日付・検索・カテゴリーによる絞り込みは維持する。

Homeの表示は `Latest`・`All articles`・`Read` に統一した。Articlesの検索UIも `Search articles…`・`Published`・`All`・`Clear filters` などに英語化し、件数は `1 article` / `2 articles` と単複を分ける。記事タイトル・本文・カテゴリー・タグの原文は保持する。日付入力には英語を指定したが、ブラウザー標準UIの年月日プレースホルダーは日本語環境では日本語表示が残る。

英語化後の検証は型チェック0 errors/warnings/hints、単体385件、統合135件、関連E2E 16件が成功。ローカルプレビューに反映済み。
